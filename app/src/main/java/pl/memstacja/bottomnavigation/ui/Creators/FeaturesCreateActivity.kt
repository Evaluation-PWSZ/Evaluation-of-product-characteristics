...
class FeaturesCreateActivity: AppCompatActivity() {

    private lateinit var name : EditText

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_features2_create)
    ...
    fun sendRequest() {
        ...
        val stringRequest = object: JsonObjectRequest(
            ...
            {
                val gson = Gson()
                val degustation: FeatureItem = gson.fromJson(it.toString(), FeatureItem::class.java)

                val intent = Intent()
                intent.putExtra("id", degustation.id)
                intent.putExtra("degustation_id", degustation.degustation_id)
